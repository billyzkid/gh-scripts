...
const groupByPath = true;

const releases = {
  name: "Unreleased",
  description: "The following commits have not been released."
};

const labels = {
  name: "Unlabeled",
  description: "The following commits have not been labeled."
};

const paths = {
  name: "Unspecified",
  description: "The following commits include unspecified files."
};
...
function getReleaseConfig(obj, allReleases) {
  ...
  return allReleases.reduce((config, release) => {
    ...
    const value = { name: release.name, release };
    ...
    releaseGroups.filter((releaseGroup) => releaseGroup.commits.length > 0).forEach((releaseGroup) => {
      ...
      if (groupByLabel && (labelGroups = getCommitsByLabel(releaseCommits, labelConfig)) && labelGroups.length > 0) {
        labelGroups.filter((labelGroup) => labelGroup.commits.length > 0).forEach((labelGroup) => {
          ...
          output += formatGroupHeading(labelGroup, 2);
          ...
          if (groupByPath && (pathGroups = getCommitsByPath(labelCommits, pathConfig)) && pathGroups.length > 0) {
            pathGroups.filter((pathGroup) => pathGroup.commits.length > 0).forEach((pathGroup) => {
              ...
              output += formatCommitsList(pathCommits, 3);
            });
          } else {
            output += formatCommitsList(labelCommits, 2);
          }
        });
        ...
        output += formatCommitsList(releaseCommits, 1);
      }
  ...
  const authors = commits.reduce((obj, commit) => {
    ...
    if (!obj[key]) {
      ...
      if (name && email) {
        ...
        sortText = `${name} (@${login})`;
        displayText = `${name} ([@${login}](${html_url}))`;
      } else {
        sortText = `@${login}`;
        displayText = `[@${login}](${html_url})`;
      }
...
function formatCommitsList(commits, level) {
  ...
  commits.forEach((commit, index, array) => {
    const head = (commit.issue) ? `[#${commit.issue.number}](${commit.issue.html_url}) - ${commit.issue.title} ([@${commit.author.login}](${commit.author.html_url}))` : `${commit.commit.message} ([@${commit.author.login}](${commit.author.html_url}))`;
...
function formatAuthorsHeading(authors) {
  return `### Contributors (${authors.length})` + EOL.repeat(2);
}
