...
function formatCommitsList(commits, level) {
  ...
  commits.forEach((commit, index, array) => {
    const head = (commit.issue) ? `[#${commit.issue.number}](${commit.issue.html_url}) - ${commit.issue.title} ${formatCommitAuthor(commit)}` : `${commit.commit.message} ${formatCommitAuthor(commit)}`;
...
function formatCommitAuthor(commit) {
  if (commit.author.login) {
    return `([@${commit.author.login}](${commit.author.html_url}))`;
  } else {
