...
const yargs = new Yargs()
  ...
  .example("gh-scripts authors -r v1.0.0.. -x robot -f ./AUTHORS -e utf8", "Generate a list of authors for the commits tagged v1.0.0 or later (excluding \"robot\" authors) and output it to the specified file (using utf-8 encoding)")
...
function run(args) {
  ...
  // get commit log
  let log = git.getLog({ revisionRange, firstParent: true, paths: commitPaths, format: "fuller", array: true });
