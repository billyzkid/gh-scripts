...
function run(args) {
  ...
  // get commit log
  const commitPaths = git.getCommitPaths();
  const commitLog = git.getCommitLog(revisionRange, { firstParent: true, paths: commitPaths, array: true });
  ...
  commitLog.forEach(item => {
    console.log(item);
    const match = item.match(/^Author: (.+)$/m);
